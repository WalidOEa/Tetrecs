...
/**
 * Custom UI component that receives current channels within the server and creates text label
 * reflecting the name and binds an event handler to each.
 */
public class LobbyList extends VBox {
  ...
  public LobbyList(ObservableMap<String, EventHandler<MouseEvent>> lobby) {
    lobby.addListener(
        (MapChangeListener<? super String, ? super EventHandler<MouseEvent>>) change -> build(
            lobby));
  }
