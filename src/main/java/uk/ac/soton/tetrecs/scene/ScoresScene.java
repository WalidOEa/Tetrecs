...
import uk.ac.soton.tetrecs.component.ScoresList;
...
public class ScoresScene extends BaseScene {
   ...
   */
  private final ObservableList<Pair<String, Integer>> scorePairsObservable = FXCollections.observableArrayList(
      scorePairs);

  /**
   * Expose observable list scorePairsObservable as property, allows for binding and addition of
   * listeners.
   */
  private final ListProperty<Pair<String, Integer>> scoresPairsObservableProperty = new SimpleListProperty<>(
      scorePairsObservable);
   ...
   */
  private final ObservableList<Pair<String, Integer>> remoteScoresObservable = FXCollections.observableArrayList(
      remoteScores);

  /**
   * Exposes observable list remoteScoresObservable as property, allowing for binding and event
   * listeners.
   */
  private final SimpleListProperty<Pair<String, Integer>> remoteScoresObservableProperty = new SimpleListProperty<>(
      remoteScoresObservable);
  ...
  public void build() {
    ...
    scoresPairsObservableProperty.addListener(
        (ListChangeListener<? super Pair<String, Integer>>) (change) ->
  ...
  /**
   * Loads scores from a given text file. If the file does not exist then create a new one and write
   * generated scores into it.
   *
  ...
  /**
   * Initialises an event handler to handle an escape keystroke. After the keystroke, the scene is
   * cleaned of any listeners and the Menu scene is initialised instead.
   */
  @Override
