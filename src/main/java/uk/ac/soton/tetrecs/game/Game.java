package uk.ac.soton.tetrecs.game;

import javafx.beans.property.*;
import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;
import uk.ac.soton.tetrecs.component.GameBlock;
import uk.ac.soton.tetrecs.event.*;
import uk.ac.soton.tetrecs.scene.Multimedia;
...
import static uk.ac.soton.tetrecs.game.GamePiece.PIECES;

/**
 * The Game class handles the main logic, state and properties of the TetrECS game. Methods to
 * manipulate the game state and to handle actions made by the player take place in this class.
 */
public class Game {
  ...
  /**
   * Initialise a new game and instantiate a new Runnable anonymous class. runnable calls to
   * gameLoop and is to be scheduled after a specified time delay in milliseconds. nextPiece is
   * assigned a piece initially.
   */
  protected void initialiseGame() {
    ...
    schedulerFuture = scheduler.scheduleAtFixedRate(runnable, getDelay(), getDelay(),
  ...
  /**
   * Handles the clearance of lines if possible. This is done by obtaining the grid makeup then
   * checking rows and columns for values > 0. For any lines to be cleared, means that blocks in
   * either a row of column or intersecting row/column have values > 0. The blocks are added to a
   * hashset and are then reset to their original value of 0. Changes are then made to score and
   * multiplier. If necessary, increment the level.
   */
  protected void afterPiece() {
    ...
    for (int i = 0; i < getCols(); i++) {
      flag = true;
        ...
        if (getRows() >= 0) {
            System.arraycopy(coordinates[i], 0, rows, 0, getRows());
        }
  ...
  /**
   * Reset state of game. Used when clearing up scenes and transition between menu and challenge
   * scenes.
   */
